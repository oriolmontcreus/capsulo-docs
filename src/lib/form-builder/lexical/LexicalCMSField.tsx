    ...
    const fetchVariables = async () => {
      try {
        ...
        if (globals && globals.data) {
          const items = Object.entries(globals.data).map(
            ([key, item]: [string, any]) => {
              let displayValue = "";
...
function AutocompletePlugin({
  ...
  onTrigger: (query: string | null, rect: DOMRect | null) => void;
}) {
...
const EditorInner: React.FC<LexicalCMSFieldProps & { value: string }> = ({
  ...
  const handleKeyboardSelect = () => {
    if (
      ...
      selectedIndex < filteredVariables.length
    ) {
                    ...
                    className={cn(
                      ...
                      multiline
                        ? unstyled
                          ...
                          : "top-2 left-3"
                        : unstyled
                          ? "top-1/2 left-0 -translate-y-1/2 whitespace-nowrap max-w-full"
                          : "top-1/2 left-3 -translate-y-1/2 whitespace-nowrap max-w-[calc(100%-24px)]",
                    )}
                  >
                    {placeholder}
                  </div>
