...
<Callout type="info">
**Key Features:**
- Define once in the globals schema
- Edit through the CMS admin interface
- Inject anywhere using `{{variableName}}` syntax
- Access in code via utility functions
- Fully localized with multi-language support
</Callout>
...
<Callout type="warn">
**Common Pitfalls:**
- Forgetting to add `.translatable()` for multi-language content
- Using overly generic variable names that cause confusion
- Not providing default values, leading to empty strings in production
</Callout>
