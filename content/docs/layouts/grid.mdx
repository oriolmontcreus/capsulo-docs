...
<TypeTable
  type={{
    columns: {
      description: "Number of columns in the grid. Can be a single number or responsive object.",
...
<TypeTable
  type={{
    gap: {
      description: "Sets the gap between grid items. Can be a single number or responsive object.",
      type: "number | ResponsiveValue",
      default: "{ base: 4 }",
    },
    contains: {
...
<ComponentPreview showMargin={false} className="col-span-full">
  <SchemaRenderer schema={ColSpanMixedFieldsSchema} />

```typescript
// ColorPicker and Switch with column spanning
Grid({ base: 1, md: 2, lg: 3 })
  .contains([
